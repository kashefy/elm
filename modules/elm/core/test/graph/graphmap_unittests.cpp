...
TEST_F(GraphMapConstructTest, Destruct)
{
    GraphMap *to = new GraphMap();
    EXPECT_NO_THROW(delete to);
}

TEST_F(GraphMapConstructTest, EmptyMap)
{
    Mat1f x;
    GraphMap to(x, Mat());
...
TEST_F(GraphMapConstructTest, SingleElementMap)
{
    Mat1i x(1, 1, 0);
    EXPECT_THROW(GraphMap to(x, Mat()), ExceptionBadDims);
    EXPECT_THROW(GraphMap to(x+1, Mat()), ExceptionBadDims);
}

TEST_F(GraphMapConstructTest, Max_num_vertices)
{
    for(int n=2; n<11; n++) {
...
TEST_F(GraphMapConstructTest, AdjacencyDense_dims_all_unique)
{
    for(int n=2; n<11; n++) {
 ...
 */
TEST_F(GraphMapConstructTest, AdjacencyDense_row_mat)
{
    Mat1i m(1, 5);
    for(size_t i=0; i<m.total(); i++) {
...
TEST_F(GraphMapConstructTest, AdjacencyDense)
{
    const int ROWS=2;
    const int COLS=3;
...
TEST_F(GraphMapConstructTest, VerticesIds_size)
{
    for(int n=2; n<11; n++) {
 ...
 */
TEST_F(GraphMapConstructTest, VerticesIds)
{
    const int ROWS=3;
    const int COLS=3;
    int data[ROWS*COLS] = {1, 1, 2,
...
TEST_F(GraphMapConstructTest, Mask)
{
    const int ROWS=2;
    const int COLS=3;
    int data[ROWS*COLS] = {1, 1, 2,
    ...
    {
        Mat mask;
        EXPECT_NO_THROW(GraphMap to(m, mask));
    }
    {
    ...
    }
    {
        ...
        EXPECT_THROW(GraphMap to(m, mask), ExceptionBadDims);
    }
    {
...
TEST_F(GraphMapConstructTest, VerticesIds_masked)
{
    const int ROWS=3;
    const int COLS=3;
    int data[ROWS*COLS] = {1, 1, 2,
